...
export const performEnhancedOCR = async (imageFile, grade = 2) => {
  try {
    ...
    // Initialize Tesseract with optimal configuration
    const worker = await createWorker();
    
    // Configure for sight word recognition
    await worker.loadLanguage('eng');
    await worker.initialize('eng');
    
    // Set parameters to optimize for sight words
    await worker.setParameters({
      tessedit_char_whitelist: 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ- ',
    });
    
    // First try with PSM 6 (assume uniform text block)
    const result1 = await worker.recognize(processedImage);
    
    // Then try with PSM 11 (sparse text with OSD)
    await worker.setParameters({
      tessedit_pageseg_mode: '11'
    });
    const result2 = await worker.recognize(processedImage);
    
    // Finally try with PSM 3 (auto with no OSD)
    await worker.setParameters({
