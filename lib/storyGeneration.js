...
const fillTemplate = (template, categorizedWords, originalWords) => {
  ...
  const selectBestWord = (category, section) => {
    ...
    if (isProperNameContext(section, category)) {
      ...
      if (categorizedWords.NAME.length === 0) {
        const randomIndex = Math.floor(Math.random() * PROPER_NAMES.length);
        return PROPER_NAMES[randomIndex];
      }
    }
    ...
    if (unusedWord) {
      ...
      if (originalWords.includes(unusedWord)) {
        wordUsageMap[unusedWord.toLowerCase()] = true;
      }
      return unusedWord;
    }
    ...
    // If it's from the original list, mark it as used
    if (originalWords.includes(randomWord)) {
      wordUsageMap[randomWord.toLowerCase()] = true;
    }
    return randomWord;
  };
  ...
  if (unusedWords.length > 0) {
    ...
    // Get some names to use if needed
    const defaultName = categorizedWords.NAME.length > 0 ? 
      categorizedWords.NAME[0] : 'Mason';
    ...
    unusedWords.forEach(word => {
      ...
      switch (category) {
        case 'ANIMAL':
          ...
          break;
        case 'PERSON':
          // Use the word as a common noun, not a name
          predicate = `talked to their ${word}.`;
          break;
        case 'NAME':
          // This is already a proper name
          predicate = `waved to ${word}.`;
          break;
        case 'PLACE':
