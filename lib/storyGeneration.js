...
const fillTemplate = (template, categorizedWords, originalWords) => {
  ...
  const selectBestWord = (category, section, placeholder = category) => {
    ...
    // Use a variety of transition words instead of just "Then"
  const transitions = ["Then", "Next", "After that", "Later", "Soon", "Meanwhile"];
  let lastTransition = "";
  ...
  storySections = storySections.map((section, index) => {
    ...
    // If the section starts with "Then", replace with varied transitions
    if (filledSection.startsWith("Then ") && lastTransition === "Then") {
      const newTransition = transitions[Math.floor(Math.random() * transitions.length)];
      filledSection = filledSection.replace("Then ", `${newTransition} `);
      lastTransition = newTransition;
    } else if (filledSection.startsWith("Then ")) {
      lastTransition = "Then";
    }
